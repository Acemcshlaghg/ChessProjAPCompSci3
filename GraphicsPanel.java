...
public class GraphicsPanel extends JPanel implements MouseListener {
	...
	private boolean click; // false until the game has started by somebody clicking on the board. should
	...
	private int turn = 1;
	private Piece[][] board = new Piece[8][8]; // an 8x8 board of 'Pieces'. Each spot should be filled by one of the
												// chess pieces or a 'space'.
	...
	public GraphicsPanel() {
		setPreferredSize(new Dimension(SQUARE_WIDTH * 8 + OFFSET + 2, SQUARE_WIDTH * 8 + OFFSET + 2)); // Set these
																										// dimensions to
																										// the width
	...
	public void paintComponent(Graphics g) {
		...
		if (fromAndToCounter == 1 && turn == 1) {
			...
			g2.fillRect(from.getRow() * 90, from.getColumn() * 90, 90, 90);
		} else if (fromAndToCounter == 1 && turn == 2) {
			g2.setColor(Color.blue);
			g2.fillRect(from.getRow() * 90, from.getColumn() * 90, 90, 90); // I don't know how big the acual sqaures
																			// are in px
		} else if (fromAndToCounter == 2) {
			fromAndToCounter = 0;
		}
		// instead of drawing a single piece you should loop through the two-dimensional
		// array and draw each piece except for
		// empty spaces.
	...
	public void mouseClicked(MouseEvent e) {
		...
		case 1:
			...
			if (fromAndToCounter == 1 && board[from.getColumn()][from.getRow()].isValidMove(from, to, board) && (from.getColumn() != to.getColumn() || from.getRow() != to.getRow())) {
