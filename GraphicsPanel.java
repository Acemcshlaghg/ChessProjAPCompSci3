...
public class GraphicsPanel extends JPanel implements MouseListener{
	...
	private int whiteKingCount = 0; //used in determining if game is over
	private int blackKingCount = 0; //used in determining if game is over
	...
	public void paintComponent(Graphics g) {
		...
		if (whiteKingCount == 1 && blackKingCount == 0) { // if black king dies...
			...
			gameOver = true;
			g2.setFont(new Font(Font.DIALOG, 2, 30));
			g2.setColor(Color.blue);
		...
		if (gameOver == false) { // if the game is over, clicking will do nothing
			...
			if (turn == 1) { // white turn
				if (fromAndToCounter == 0) { // if the user clicks on a "from piece" piece
					...
					if (board[from.getRow()][from.getColumn()].isValidMove(from, to, board) == true && from.equals(to) == false && board[from.getRow()][from.getColumn()].getPlayer() == 1) { // if the move is valid
				...
				if (fromAndToCounter == 0) { // if the user clicks on a "from piece" piece
					...
					to = new Location(e.getX()/90, e.getY()/90);			
					if (board[from.getRow()][from.getColumn()].isValidMove(from, to, board) == true && from.equals(to) == false && board[from.getRow()][from.getColumn()].getPlayer() == 2) { // if the move is valid
						board[to.getRow()][to.getColumn()] = board[from.getRow()][from.getColumn()]; // this moves the piece to its new location
						board[from.getRow()][from.getColumn()] = null; // sets old spot to null
						fromAndToCounter++;	
